...
const NoteDetailsPage = () => {
    ...
    const [noteUpdateState, noteUpdateAction] = useActionState(updateNote, null);
    ...
    if (!note) {
        return (
            <div className="min-h-screen flex flex-row flex-1 items-center justify-center ">
                <svg className="mr-3 size-7 animate-spin" viewBox="0 0 24 24" aria-hidden="true">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8v4a4 4 0 0 0-4 4H4z" />
                </svg>
                <p className="text-3xl tracking-tight text-indigo-300">Loading note...</p>
            </div>
        )
    }
    ...
    return (
        <section className="relative mx-auto max-w-3xl p-6">
            <nav className="mb-4 text-sm text-gray-400">
                <Link className="hover:text-gray-200 transition-colors duration-200" href="/dashboard">← Back to Dashboard</Link>
            </nav>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl shadow-[0_10px_50px_rgba(0,0,0,0.35)]">
                ...
                <header className="mb-4 flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2.5">
                        ...
                        <form /* action={summarizeNote} */>
                            ...
                            <SummarizeButton pendingText="Summarizing…">Summarize</SummarizeButton>
                        </form>

                        <form action={noteDeleteAction}>
                            <input type="hidden" name="id" value={note.id} />
                            <DeleteButton isNoteDeleting={isNoteDeleting} text="Delete" noteId={note.id} />
                        </form>
                    </div>
                </header>
                ...
                <form action={noteUpdateAction} className="grid gap-4">
                    ...
                    <div className="flex justify-end pt-2 w-3/4 mx-auto">
